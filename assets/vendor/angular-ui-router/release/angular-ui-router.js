...
var UrlRuleFactory = /** @class */ (function () {
     ...
     * var rule = factory.fromUrlMatcher(urlMatcher, match => "/movie-list/" + match.fooId + "/" + match.barId);
     * var match = rule.match('/foo/123/456'); // results in { fooId: '123', barId: '456' }
     * var result = rule.handler(match); // '/movie-list/123/456'
     * ```
     *
     ...
     * ```js
     * var urlMatcher = $umf.compile("/foo/:fooId/:barId");
     * var handler = $umf.compile("/movie-list/:fooId/:barId");
     ...
     * #### Example:
     * ```js
     * var rule = factory.fromRegExp(/^\/foo\/(bar|baz)$/, match => "/movie-list/" + match[1])
     * var match = rule.match('/foo/bar'); // results in [ '/foo/bar', 'bar' ]
     * var result = rule.handler(match); // '/movie-list/bar'
     * ```
     *
     * ## Handler as string
     *
     ...
     * #### Example:
     * ```js
     * var rule = factory.fromRegExp(/^\/foo\/(bar|baz)$/, "/movie-list/$1")
     * var match = rule.match('/foo/bar'); // results in [ '/foo/bar', 'bar' ]
     * var result = rule.handler(match); // '/movie-list/bar'
     * ```
     */
...
var StateService = /** @class */ (function () {
     ...
     * - `$state.go('^.sibling')` - if current state is `movie-list.child`, will go to the `movie-list.sibling` state
     * - `$state.go('.child.grandchild')` - if current state is movie-list, will go to the `movie-list.child.grandchild` state
     *
...
var StateProvider = /** @class */ (function () {
     ...
     *
     * ```js
     * // Auto-populates list and item views with /partials/movie-list/contact/list.html,
     * // and /partials/movie-list/contact/item.html, respectively.
     * $state.go('movie-list');
     * ```
     *
 ...
 *
 * #### Example:
 * This will activate the `movie-list` state when the link is clicked.
 * ```html
 * <a ui-sref="movie-list">Home</a>
 * ```
 *
 * ### Relative Links
 ...
 * #### Example:
 * ```html
 * <a ui-sref="movie-list" ui-sref-opts="{ reload: true }">Home</a>
 * ```
 *
 * ### Other DOM Events
 *
 ...
 *
 * ```html
 * <a href="#/movie-list" ui-sref="movie-list">Home</a>
 ...
 * </ul>
 *
 * <a href="#/movie-list" ui-sref="movie-list" ui-sref-opts="{reload: true}">Home</a>
 * ```
 *
 * ### Notes
 *
